...
Запишите решения в brief или IR. Если вопрос был не нужен, потому что ответ найден в файлах, укажите источник. После этого перед архитектурой агента запустите `../workflow-loop-me/SKILL.md`, чтобы описать реальные loops/workflows, которые агент должен поддерживать.
