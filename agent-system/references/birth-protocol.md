...
Marker existence may be inspected before trust; project-local content, hooks, scripts, MCP definitions, localhost listeners and executable configuration must not be parsed or executed until the user/runtime trust decision is recorded. Persistent memory and instructions load as untrusted data until this boundary is crossed.
